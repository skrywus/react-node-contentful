...
import {getArchiveLeagues, areLeaguesLoaded, isLoaderVisible, getError} from '../../../../state/league/selectors';
...
const ArchiveLeagues = ({areLeaguesLoaded, isLoaderVisible, leagues, error}) => {
    return(
        <Layout>
                                ...
                                return(
                                    <Col key={index} md={8} className="col-md-offset-2 league-wrapper">
                                        <div className="leagueName text-left">
                                            <a href={'#/archive/' + league.sys.id}>{league.fields.name}</a>
...
const mapStateToProps = (state) => {
    return {
        leagues: getArchiveLeagues(state),
...
export default connect(mapStateToProps, mapDispatchToProps)(ArchiveLeagues);
